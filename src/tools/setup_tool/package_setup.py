...
import json
from tools.string_utils import remove_triple_quotes as DevinciStringUtils
...
class PackageSetup:
    ...
    def  generate_setup_py(self, config_filename='config.ini',
        ...
        with open(setup_stub_filename, 'r') as stub_file:
            ...
            setup_stub_content = DevinciStringUtils.process_between_delimiters(
                content, '"""')

            print(setup_stub_content, type(setup_stub_content))
        ...
        setup_content = setup_stub_content.format(
            ...
            data_files=self.parse_data_files(
                self.get_config_value('DataFiles')),
            scripts=self.parse_scripts(
                self.get_config_value('Scripts')),
            py_modules=self.parse_modules(
                self.get_config_value('PyModules'))
        )
